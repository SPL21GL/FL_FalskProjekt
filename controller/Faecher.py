...
def deleteFach():
    ...
    if deleteLehrerformObj.validate_on_submit():
        ...
        itemToDelete = db.session.query(Faecher).filter(
            Faecher.Faecher_Id == itemIdToDelete)
        itemToDelete.delete()
...
def submitEditForm():
    ...
    if editItemFormObject.validate_on_submit():
        ...
        item_to_edit = db.session.query(Faecher).filter(
            Faecher.Faecher_Id == itemId).first()
...
def showEditForm():
    ...
    item_to_edit = db.session.query(Faecher).filter(
        Faecher.Faecher_Id == Faecher_Id).first()
